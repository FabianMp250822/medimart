...
export default function PostularmeModal({ show, onClose, oferta }) {
  ...
  const [tieneRethus, setTieneRethus] = useState("No");
  ...
  const handleSubmit = async (e) => {
    ...
    const postulacion = {
      id: postulacionId,
      ofertaId: oferta.id || "",
      ofertaTitulo: oferta.titulo || "Sin Oferta",
    ...
    try {
      ...
      let q;
      if (!oferta.id || oferta.titulo.trim().toLowerCase() === "sin oferta") {
        q = query(
          postulacionesRef,
          where("ofertaTitulo", "==", "Sin Oferta"),
          where("informacionPersonal.correoElectronico", "==", normalizedEmail)
        );
      } else {
        q = query(
          postulacionesRef,
      ...
      if (!querySnapshot.empty) {
        Swal.fire({
          ...
          title: "Error",
          text:
            !oferta.id || oferta.titulo.trim().toLowerCase() === "sin oferta"
              ? "Ya has enviado tu currículum a la clínica."
              : "Ya te has postulado a esta oferta con este correo electrónico.",
      ...
      const response = await fetch(
        "https://us-central1-clinica-de-la-costa.cloudfunctions.net/enviarCorreoPostulacion",
        {
          method: "POST",
          headers: {
            ...
            <div className="col-md-6">
              ...
              <div className="form-group">
                <label htmlFor="numeroDocumento">
                  Número de {tipoDocumento === "tarjeta" ? "Tarjeta Profesional" : "Resolución"}:
                </label>
                <input
                  type="text"
                  id="numeroDocumento"
